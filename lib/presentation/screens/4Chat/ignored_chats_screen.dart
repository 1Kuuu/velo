...
class IgnoredChatsScreen extends StatelessWidget {
  const IgnoredChatsScreen({Key? key}) : super(key: key);
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, prefsSnapshot) {
          ...
          if (!prefsSnapshot.hasData || !prefsSnapshot.data!.exists) {
            return _buildEmptyState(isDarkMode, "No ignored chats", "Ignored conversations will appear here");
          }

          final chatPrefs = prefsSnapshot.data!.data() as Map<String, dynamic>?;
          final List<String> ignoredChats = List<String>.from(chatPrefs?['ignored_chats'] ?? []);

          if (ignoredChats.isEmpty) {
            return _buildEmptyState(isDarkMode, "No ignored chats", "Ignored conversations will appear here");
          }

          return ListView.builder(
            itemCount: ignoredChats.length,
            padding: const EdgeInsets.all(16),
            itemBuilder: (context, index) {
              final chatId = ignoredChats[index];
        ...
        return Dismissible(
          ...
          confirmDismiss: (direction) async {
            if (direction == DismissDirection.endToStart) {
              ...
              // Unignore without confirmation
              _unignoreConversation(chatId);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Conversation with $name visible again')),
              );
              return true;
            }
          },
